...
window.Sidebar = function({ active, setActive }) {
  return (
    <aside className="w-[220px] min-h-screen border-r border-slate-200 flex flex-col py-6 relative z-10" style={{ background: "var(--bg-sidebar)" }}>
      <div className="px-5 mb-8">
        <div className="text-[24px] font-extrabold text-white leading-tight drop-shadow-md mt-2">Flash Studio</div>
      </div>
      ...
      <div className="px-5 py-5 border-t border-slate-800 bg-slate-900/30">
        <div className="mb-4">
          <div className="text-[14px] text-white font-extrabold tracking-wide mb-1">Diagnostic Tool</div>
          <div className="text-[9px] text-slate-400 leading-snug uppercase tracking-wider">by MAHLE ANAND THERMAL SYSTEM Pvt. Ltd.</div>
        </div>
        <div className="text-[10px] text-slate-500 mb-1.5 font-medium uppercase tracking-wider">Backend Status</div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse-slow shadow-[0_0_8px_rgba(52,211,153,0.8)]"></span>
          <span className="text-[11px] text-emerald-400 font-bold">Python API Live</span>
        </div>
      </div>
    </aside>
  );
