...
window.CanTrace = function() {
  ...
  const getRowStyle = (f) => {
    ...
    if (data.startsWith("36") || data.startsWith("34")) return { background: "rgba(59, 130, 246, 0.08)", borderLeft: "4px solid #3b82f6" };
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 12, padding: "0 10px" }}>
            {[["TX", showTX, setShowTX], ["RX", showRX, setShowRX], ["EVT", showEVT, setShowEVT]].map(([l, v, s]) => (
              <label key={l} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: v ? "var(--text-primary)" : "#475569", cursor: "pointer", fontWeight: v ? 700 : 400 }}>
                <input type="checkbox" checked={v} onChange={e => s(e.target.checked)} className="accent-blue-500" /> {l}
              </label>
            ))}
          </div>
          <div style={{ borderLeft: "1px solid #334155", height: 20, margin: "0 10px" }} />
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: autoScroll ? "var(--text-primary)" : "#475569", cursor: "pointer" }}>
            <input type="checkbox" checked={autoScroll} onChange={e => setAutoScroll(e.target.checked)} /> Auto-scroll
          </label>
          <div style={{ display: "flex", gap: 6 }}>
            <window.Btn onClick={copyLog} color="#334155">Copy</window.Btn>
            <window.Btn onClick={exportLog} color="#334155">Export</window.Btn>
      ...
      <window.Card style={{ padding: 0 }}>
        ...
        <div ref={wrapRef} style={{ maxHeight: 520, overflowY: "auto", overflowX: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ color: "#64748b", borderBottom: "1px solid #334155", position: "sticky", top: 0, background: "rgba(15, 23, 42, 0.95)", backdropFilter: "blur(4px)", zIndex: 10 }}>
            ...
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={5} style={{ textAlign: "center", color: "#475569", padding: 32, fontSize: 13 }}>No frames match filters.</td></tr>
                : filtered.map((f, i) => (
                  <tr key={f.id || i} style={{ ...getRowStyle(f), borderBottom: "1px solid rgba(51,65,85,0.2)", transition: "all 0.2s" }} className="group">
                    ...
                    <td style={{ padding: "10px 14px", fontFamily: "monospace", color: f.data.startsWith("7F") ? "#fca5a5" : "#e2e8f0", letterSpacing: "0.05em" }}>{f.data}</td>
                    <td style={{ padding: "10px 14px", fontSize: 11, color: "#94a3b8", fontStyle: f.note ? "normal" : "italic" }}>{f.note || "—"}</td>
                  </tr>
                ))
              }
            </tbody>
